...
export function getFileList(dirName, og = dirName, ignored = new Set()) {
    return __asyncGenerator(this, arguments, function* getFileList_1() {
        const items = yield __await(readdir(dirName, { withFileTypes: true }));
        ignored.add('.git');
        for (const item of items) {
            ...
            if (item.isDirectory()) {
                yield __await(yield* __asyncDelegator(__asyncValues(getFileList(`${dirName}/${item.name}`, og))));
            }
            else {
                const path = `${dirName}/${item.name}`;
                yield yield __await(path.startsWith(og) ? path.substring(og.length) : path);
            }
        }
    });
}
