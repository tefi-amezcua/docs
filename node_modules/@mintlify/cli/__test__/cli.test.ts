...
// TODO properly transform ts dependencies insteead of mocking: try using babel-jest
...
jest.mock('@mintlify/link-rot', () => {
  return {
    getBrokenInternalLinks: (_: string) => console.log(`broken-links`),
    renameFilesAndUpdateLinksInContent: (_from: string, _to: string) => console.log(`rename`),
  };
});
...
describe('cli', () => {
  ...
  it('port 5000 and 5001 should be taken and 5002 should be accepted by the user and available.', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      `Port 5000 is already in use. Use port 5001 instead? [Y/n]\n`
    );
    expect(consoleSpy).toHaveBeenCalledWith(
      `Port 5001 is already in use. Use port 5002 instead? [Y/n]\n`
    );
